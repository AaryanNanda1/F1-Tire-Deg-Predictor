...
def _git_revision() -> str:
    """Return the source revision used to produce a manifest, if available."""
    configured = os.environ.get("GITHUB_SHA") or os.environ.get("SOURCE_GIT_COMMIT")
    if configured:
...
class ProcessedSessionStore:
    ...
    def _empty_manifest(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "preprocessing_git_commit": _git_revision(),
            "format": "csv.gz",
            "description": (
    ...
    def requires_rebuild(self) -> bool:
        """Whether stored rows were generated by an older data contract."""
        return (
            self._manifest.get("schema_version") != SCHEMA_VERSION
            or self._manifest.get("preprocessing_version") != PREPROCESSING_VERSION
        )

    @property
    def manifest_schema_version(self) -> int:
        return int(self._manifest.get("schema_version", 0))

    def require_current_schema(self) -> None:
        if self.requires_rebuild:
            raise TrainingDataStoreError(
                "Processed training data is stale: "
                f"manifest schema={self.manifest_schema_version}, "
                f"preprocessing={self._manifest.get('preprocessing_version')!r}; "
                "run scripts/rebuild_processed_training_data.py before training."
            )
    ...
    def save_session(
        ...
        *,
        ...
        """Save a processed session, optionally replacing a stale row."""
        if self.has_session(spec.key) and not overwrite:
            return False
        ...
        filter_audit = output.attrs.get("filter_audit")
        if isinstance(filter_audit, dict):
            self.sessions[spec.key]["filter_audit"] = filter_audit
        self._write_manifest()
        return True
    ...
    def load_role(self, role: str) -> pd.DataFrame:
        ...
        for entry in entries:
            ...
            # Restore immutable event metadata from the manifest so
            # chronological validation can keep repeated annual events in
            # distinct walk-forward folds. Categorical model columns remain
            # raw; encoding is fitted by the training pipeline.
...
def refresh_sessions(
    ...
    *,
    ...
    """Download/process sessions, optionally replacing stale existing rows."""
    if store.requires_rebuild and not reprocess_existing:
    ...
    for spec in specs:
        was_present = store.has_session(spec.key)
        if was_present and not reprocess_existing:
            skipped.append(spec.key)
            continue
        try:
            session = loader(spec.year, spec.event_name, spec.session_code)
            frame = preprocessor(session)
            store.save_session(spec, frame, overwrite=reprocess_existing)
            if reprocess_existing and was_present:
                reprocessed.append(spec.key)
            else:
